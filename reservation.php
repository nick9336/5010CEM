   ...
   <?php
        if ($user_id == '') {
            ...
            if (isset($_POST['submit'])) {
                ...
                $user_name = $conn->prepare("SELECT * FROM `users` WHERE id = 6");
                $user_name->execute();
                if($user_name->rowCount() > 0){
                     $fetch_username = $user_name->fetch(PDO::FETCH_ASSOC);
                    $name = $fetch_username['name'];
                }
                ...
                if (!empty($name) && !empty($email) && !empty($place_on) && !empty($time_place) && !empty($pax) && !empty($contact_no)) {
                    ...
                    $insert_res = $conn->prepare("INSERT INTO reservations (user_id, name, placed_on, time_placed, pax, contact_no, email, reservation_status)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
                    $insert_res->execute([$user_id, $name , $place_on, $time_place, $pax, $contact_no, $email, 'Pending']);
